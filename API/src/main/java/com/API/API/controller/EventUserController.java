...
public class EventUserController {
    ...
    @GetMapping("/paginatedEventUsers")
    public ResponseEntity<Page<EventUser>> getPaginatedEventUsers(
            @RequestParam(defaultValue = "0") int page,  // Trang mặc định là 0
            @RequestParam(defaultValue = "10") int size // Kích thước mặc định là 10
    ) {
        Page<EventUser> paginatedEventUsers = eventUserService.getPaginatedEventUsers(page, size);
        return ResponseEntity.ok(paginatedEventUsers);
    }
